...
int main(int argc, char *argv[])
{
    ...
    /* start threads to receive traffic  commented Dj*/
    for (i = 0; i < num_server; i++)
    {
...
void print_statistic()
{
    ...
    for (i = 0; i < req_total_num; i++)
    {
        ...
        if (fct_us > 0)
            ...
            flow_goodput_mbps = 0;
        //fprintf(fd_log, "Size:%u, Duration(usec):%u\n",size,duration); output format from emperical 
        /* size (bytes), FCT(us), DSCP, sending rate (Mbps), goodput (Mbps) */
        //fprintf(fd, "Size:%u, Duration(usec):%llu,DCCP:%u,Sendingrate(Mbps):%u,Goodput(Mbps):%u\n", req_size[i], fct_us, req_dscp[i], req_rate[i], flow_goodput_mbps);
  
  fprintf(fd, "Size:%u, Duration(usec):%llu\n", req_size[i], fct_us);
